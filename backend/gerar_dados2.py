...
# NOTA: Franquia 1 (Matriz) NÃO VENDE, então não contratamos caixas para ela.
print("   -> Franquia 1 (Matriz): Pulada (Não realiza vendas diretas).")

# A Franquia 2 (SBC) Vende, então precisa de equipe.
...
# AQUI ESTÁ A MUDANÇA: A lista de lojas só contém a Franquia 2
lojas = [
...
for dia in range(total_dias + 1):
    ...
    # Domingo loja fecha? (30% de chance de abrir domingo, 95% dias normais)
    ...
    for loja in lojas:
        if random.random() < chance_abrir:
            ...
            # Se for hoje e ainda não abriu, pula
...
print(f"\n✨ PRONTO! Simulamos vendas para a Filial SBC até: {agora}")
print(f"💰 Vendas geradas: {vendas_totais}")
print(f"ℹ️  Matriz (ID 1) foi ignorada nas vendas.")